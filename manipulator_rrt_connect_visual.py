...
# 시각화: FK 기반 노드 점 찍기
def draw_node_fk_point(robot_id, joint_indices, joint_angles, end_effector_link_index):
    pos = get_end_effector_pos(robot_id, joint_indices, joint_angles, end_effector_link_index)
    #p.addUserDebugLine(pos, pos + np.array([0, 0, 0.01]), [1, 0, 0], 2, 1.5)
    p.addUserDebugLine(pos, pos + np.array([0, 0, 0.01]), [1, 0, 1], 2)
...
def extend_toward_sample(from_tree, sample_pos, step_size, robot_id, joint_indices, obstacle_ids, end_effector_link_index, max_extend_steps=10):
    ...
    for step in range(1, max_extend_steps + 1):
        ...
        draw_node_fk_point(robot_id, joint_indices, new_node.joint_pos, end_effector_link_index)
        last_node = new_node
...
def manipulator_rrt_connect(robot_id, joint_indices, obstacle_ids, target_pos, target_ori,
    ...
    draw_node_fk_point(robot_id, joint_indices, start_joint, end_effector_link_index)
    draw_node_fk_point(robot_id, joint_indices, goal_joint, end_effector_link_index)
    ...
    for i in range(max_iter):
        ...
        ee_pos_a = get_end_effector_pos(robot_id, joint_indices, new_node_a.joint_pos, end_effector_link_index)
        new_node_b = extend_toward_sample(tree_b, ee_pos_a, step_size, robot_id, joint_indices, obstacle_ids, end_effector_link_index, max_extend_steps)
